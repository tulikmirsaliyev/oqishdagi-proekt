...
const Product = () => {
  ...
  const filterProduct= (cat) =>{
    const updatedList = data.filter((x)=>x.categry === cat);
    ...
    return (
      <>
        <div className="buttons d-flex justify-content-center mb-4">
          ...
          <button className="btn btn-outline-primary me-2" onClick={()=>filterProduct("jewelery")}>Jewelery Clothing</button>
          <button className="btn btn-outline-primary me-2" onClick={()=>filterProduct("electronics")}>Electronic Clothing</button>
        </div>
        {filter.map((product) => {
          return (
            <>
      ...
      <div className="container my-5 py-5">
        <div className="row">
          <div className="col-12 mb-5">
            <h1 className='display-6 display-justify-center text-center fw-bolder text-center'>Latest Products</h1>
            <hr />
          </div>
        </div>
